...
// Q6处理函数：只统计，不聚合 (Arrow版本)
void q6_filter_stats_collector(const ArrowBatch *batch, const SelVec *sel,
                               void *stats_ptr) {
  Q6FilterStats *stats = (Q6FilterStats *)stats_ptr;
...
void q6_stats_finalize(void *stats_ptr, void *result) {
  Q6FilterStats *stats = (Q6FilterStats *)stats_ptr;

  printf("\n=== Q6 Arrow Scan & Filter Performance ===\n");
...
int main(int argc, char *argv[]) {
  printf("=== TPC-H Q6 Arrow Scan & Filter Test ===\n");
  ...
  // 初始化统计收集器
  Q6FilterStats stats = {0};

  AggCallback stats_callback = {.process_batch = q6_filter_stats_collector,
                                .finalize = q6_stats_finalize,
                                .agg_state = &stats};

  // 测量Q6批循环性能
  printf("\nStarting Q6 Arrow batch processing...\n");
  clock_t batch_start = clock();

  int32_t result = ndb_q6_batch_driver(scan_handle, &stats_callback);
  ...
  if (result >= 0) {
    ...
    if (stats.total_scan_time > 0) {
      ...
      double throughput_mb = (stats.total_rows_scanned * sizeof(int64_t) * 16) /
                             (1024 * 1024 * stats.total_scan_time);
      printf("Throughput: %.0f rows/sec, %.1f MB/sec\n", throughput_rows,
             throughput_mb);
    }

    printf("Q6 Arrow scan & filter test completed successfully\n");
  } else {
