...
int main(int argc, char *argv[]) {
  printf("=== TPC-H Q1 Aggregation Integration Test ===\n");
  ...
  if (!q1_callback) {
    printf("Failed to create Q1 aggregation callback\n");
    rt_scan_close(scan_handle);
    return 1;
  }

  // 测量Q1批循环性能（包含聚合）
  printf("\nStarting Q1 batch processing with aggregation...\n");
  ...
  if (result >= 0) {
    // 触发最终聚合结果输出（包括排序）
    q1_callback->finalize(q1_callback->agg_state, NULL);

    printf("\nTotal processing time: %.3f seconds\n", total_time);
    printf("Q1 aggregation test completed successfully\n");
  } else {
    printf("Q1 test failed with error: %d\n", result);
  }

  // 清理
  destroy_agg_callback(q1_callback);
  rt_scan_close(scan_handle);
