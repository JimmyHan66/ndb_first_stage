...
// Q1 优化列索引映射 (7列)：
// l_quantity=0, l_extendedprice=1, l_discount=2, l_tax=3, l_returnflag=4, l_linestatus=5, l_shipdate=6

// 从q1_incremental.c复制的辅助函数
...
// Q1 增量聚合实现 - 优化列版本
Q1AggregationState* q1_agg_init_optimized(void) {
...
void q1_agg_process_batch_optimized(Q1AggregationState *state, struct ArrowArray *arr,
    ...
    // Q1 优化列索引 (7列)
    int quantity_col = 0, price_col = 1, discount_col = 2, tax_col = 3;
    int returnflag_col = 4, linestatus_col = 5;  // l_shipdate=6 (聚合不需要)
...
void q1_agg_finalize_optimized(Q1AggregationState *state) {
    if (state->finalized) {
        return; // 已经finalize过了
    }
    ...
    // 输出结果
    printf("\n=== Q1 Optimized Aggregation Results ===\n");
...
void q1_agg_destroy_optimized(Q1AggregationState *state) {
    if (state) {
