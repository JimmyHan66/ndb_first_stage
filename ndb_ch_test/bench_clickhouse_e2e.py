...
- 每轮：新进程 --multiquery（带相同 settings）
  DROP/CREATE ENGINE=Memory（7列 schema）→ 从 Parquet INSERT → 执行 1 次查询
- 记录整段墙钟；重复 7 轮取平均
...
def run_scale(parquet_path: str, mem_gb: int):
...
def write_out(all_rows, scales, mem_gb):
    ...
    with open(SUMMARY_CSV, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["scale","system","mode","case","repeats","mem_gb","avg_ms"])
