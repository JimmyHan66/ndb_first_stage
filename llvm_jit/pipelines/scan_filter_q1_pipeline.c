...
// scan_filter_q1 完整 JIT 流水线
// 功能：扫描 + Q1 过滤（不做聚合）
// 输入：ScanHandle*
// 输出：过滤后的总行数
int32_t scan_filter_q1_pipeline_jit(ScanHandle *scan_handle) {
    if (!scan_handle) {
        return -1;
    }
    ...
    printf("=== scan_filter_q1 JIT Pipeline ===\n");
    ...
    while ((result = rt_scan_next(scan_handle, &arrow_batch)) > 0) {
        ...
        if (filtered_count >= 0) {
            ...
            if (batch_count % 100 == 0) {
                printf("Batch %d: %lld rows -> %d filtered (%.2f%%)\n",
                       batch_count, arrow_batch.num_rows, filtered_count,
